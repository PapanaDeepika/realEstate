...
function MarketingAgentCustomerPropertyDeals({ route }) {
          ...
          <View style={styles.detailsContainer}>
            ...
            {deal.deal.propertyType === "Layout" && (
              <View style={styles.detailItem}>
                ...
                <Text style={styles.detailText}>
                  ...
                  {i18n.t(deal.property.layoutDetails.address.state)}
                </Text>
              </View>
            )}
            ...
            {deal.property.propertyType === "Agricultural land" && (
              <View style={styles.detailItem}>
                <Entypo name="location-pin" size={20} color="#057ef0" />
                <Text style={styles.detailText}>
                  {i18n.t(deal.property?.address?.district)},{" "}
                  {i18n.t(deal.property?.address?.state)}
                </Text>
              </View>
            )}
            ...
            {deal.deal.propertyType === "Commercial" && (
              <View style={styles.detailItem}>
                ...
                <Text style={styles.detailText}>
                  {
                    i18n.t(deal.property?.propertyDetails?.landDetails?.address?.district)
                  }
                  ,{" "}
                  {i18n.t(deal.property?.propertyDetails?.landDetails?.address?.state)}
                </Text>
              </View>
            )}
          </View>
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `https://real-estate-back-end-y58p-git-main-pindu123s-projects.vercel.app/deal/customerDealsFilter/${customer.customer.deal.customerId}?text=${query}`,
        {
          method: "GET",
          headers: {
  ...
  const getDeals = useCallback(async () => {
    try {
      ...
      const response = await fetch(
        `https://real-estate-back-end-y58p-git-main-pindu123s-projects.vercel.app/deal/getCustomerDeals/${customer.customer.deal.customerId}`,
        {
          method: "GET",
          headers: {
            Authorization: `Bearer ${token}`,
          ...
          <View style={styles.searchContainer}>
            <TextInput
              placeholder={getTruncatedPlaceholder(
                i18n.t("Search By Name,Location")
              )}
          ...
          {property.length === 0 && (
            <>
              ...
              >
                ...
                <Text>{i18n.t("No Properties found")}</Text>
              </View>
            </>
          )}
          {!searched && (
...
const styles = StyleSheet.create({
  ...
  closeButton: {
    ...
    fontFamily: "Montserrat_600emiBold",
