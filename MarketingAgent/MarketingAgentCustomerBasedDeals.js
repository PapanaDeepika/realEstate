    ...
    const PropertyCard = ({ deal }) => {
        ...
        return (
            <Card style={[styles.card, { borderLeftWidth: 5, borderLeftColor: cardBorderColor }]}>
                ...
                {deal.deal.propertyType === 'Commercial' && (
                             <Image
                             source={{ uri: deal?.property?.propertyDetails?.uploadPics[0] || "https://res.cloudinary.com/ddv2y93jq/image/upload/v1735999776/g2aqcqkd1ovsqiquwmhm.jpg"  }}  // Assuming you have the URL for the property image
                             style={styles.propertyImage}
                           />
                        )}
                        {deal.deal.propertyType === 'Agricultural land' && (
                             <Image
                             source={{ uri: deal?.property?.landDetails?.images[0] || "https://res.cloudinary.com/ddv2y93jq/image/upload/v1735999776/g2aqcqkd1ovsqiquwmhm.jpg"  }}  // Assuming you have the URL for the property image
                             style={styles.propertyImage}
                           />
                        )}
                          {deal.deal.propertyType === 'Layout' && (
                             <Image
                             source={{ uri: deal?.property?.uploadPics[0] || "https://res.cloudinary.com/ddv2y93jq/image/upload/v1735999776/g2aqcqkd1ovsqiquwmhm.jpg"  }}  // Assuming you have the URL for the property image
                             style={styles.propertyImage}
                           />
                        )}
    {deal.deal.propertyType === 'Residential' && (
                             <Image
                             source={{ uri: deal?.property?.propPhotos[0] || "https://res.cloudinary.com/ddv2y93jq/image/upload/v1735999776/g2aqcqkd1ovsqiquwmhm.jpg"  }}  // Assuming you have the URL for the property image
                             style={styles.propertyImage}
                           />
                        )}
 ...
 const customer = route.params
    ...
    const handleSearch =async()=>{
        ...
        try {
            ...
            const response = await fetch(`http://172.17.13.106:3000/deal/customerDealsFilter/${customer.customer.deal.customerId}?text=${query}`, {
                method: "GET",
                headers: {
...
//             }
    ...
    const getDeals = useCallback(async () => {
        try {
            ...
            const response = await fetch(`http://172.17.13.106:3000/deal/getCustomerDeals/${customer.customer.deal.customerId}`, {
                method: "GET",
                headers: {
                    Authorization: `Bearer ${token}`,
                    "Content-Type": "application/json",
