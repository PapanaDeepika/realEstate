...
export function PushNotificationProvider({ children }) {
    ...
    useEffect(() => {
        ...
        notificationListener.current = Notifications.addNotificationReceivedListener((notification) => {
            ...
            Alert.alert("Received message for foreground",JSON.stringify(notification?.request?.content?.data))
            setNotification(notification);
        ...
        responseListener.current = Notifications.addNotificationResponseReceivedListener((response) => {
            ...
            Alert.alert("Received message for background",JSON.stringify(response?.request?.content?.data))

              setNotification1(response)
             handleNotificationResponse(response);
    ...
    return (
        <PushNotificationContext.Provider value={{ expoPushToken, notification,  handleNotificationResponse, notification1 }}>
            {children}
