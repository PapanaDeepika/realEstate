...
function BuyerRequests() {
  ...
  const getRequests =  async () => {
    try {
      ...
      console.log("pageess",page)

      const response = await fetch(
        `https://real-estate-back-end-y58p-git-main-pindu123s-projects.vercel.app//booking/getbookingsbystatus/3/3?page=${page}&limit=8`,
        {
          method: "GET",
          headers: {
      ...
      if (data.length > 0) {
        ...
        setFilterData((prevData) => [...prevData, ...data]);
        setRequests((prevData) => [...prevData, ...data]);
