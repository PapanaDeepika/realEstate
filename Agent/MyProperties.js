...
function MyProperties() {
  ...
  const fetchProperties = async () => {
    try {
      ...
      const response = await fetch(
        `https://real-estate-back-end-y58p-git-main-pindu123s-projects.vercel.app//property/getpropbyid/${userId}?page=${page}&&limit=8`,
        {
          method: "GET",
          headers: {
  ...
  const handleShare = async (property) => {
    ...
    if (property.propertyType === "Agricultural land") {
      ...
      propPrice = handlePriceFormat(
        property.landDetails?.price || property.price
      );
      ...
      propPrice = handlePriceFormat(
        property.layoutDetails?.plotPrice || property.price
      );
      propLocation =
        property.layoutDetails?.address?.district || property.address;
      ...
      propTitle =
        property.propertyDetails?.apartmentName || property.propertyTitle;
      propPrice = handlePriceFormat(
        property.propertyDetails?.flatCost || property.price
      );
      propLocation = property.address?.district || property.address;
    }
        ...
        >
          {(item.propertyType === "Agricultural land" ||
            ...
            <Text style={styles.imageText}>
              {item.landDetails?.title || item.propertyTitle}@
              {item.propertyType}
            </Text>
          )}
          {item.propertyType === "Agricultural land" && (
            <Text style={styles.priceBottomStyle}>
              {handlePriceFormat(item.landDetails?.price || item.price)}{" "}
            </Text>
          )}
          <TouchableOpacity
      ...
      {item.propertyType === "Residential" && (
        ...
        >
          {(item.propertyType === "Residential" ||
            item.propertyType === "residential") && (
            <Text style={styles.imageText}>
              {item.propertyDetails?.apartmentName || item.propertyTitle}@
              {item.propertyType}
            </Text>
          )}
          {(item.propertyType === "Residential" ||
            item.propertyType === "residential") && (
            <Text style={styles.priceBottomStyle}>
              {handlePriceFormat(item.propertyDetails?.flatCost || item.price)}{" "}
            </Text>
          )}
          <TouchableOpacity
            style={styles.shareIcon}
      ...
      {item.propertyType === "Commercial" && (
        ...
        >
          {(item.propertyType === "Commercial" ||
            item.propertyType === "commercial") && (
            <Text style={styles.imageText}>
              {item.propertyTitle || item.propertyTitle}@{item.propertyType}
            </Text>
          )}
      ...
      {item.propertyType === "Layout" && (
        ...
        >
          {(item.propertyType === "Layout" ||
            item.propertyType === "layout") && (
            <Text style={styles.imageText}>
              {item.layoutDetails?.layoutTitle || item.propertyTitle}@
              {item.propertyType}
            </Text>
          )}
          {item.propertyType === "Layout" && (
            <Text style={styles.priceBottomStyle}>
              {handlePriceFormat(item.layoutDetails?.plotPrice || item.price)}{" "}
            </Text>
          )}
          <TouchableOpacity
            style={styles.shareIcon}
      ...
      <View style={styles.detailsContainer}>
        <View style={styles.detailsStyles}>
          ...
          {item.propertyType === "Agricultural land" && (
            <>
              <Text style={styles.textStyleNew}>
                {i18n.t(item.address?.district || item.address)}
              </Text>
            </>
          )}
          {(item.propertyType === "Residential" ||
          ...
          {item.propertyType === "Layout" && (
            <Text style={styles.textStyleNew}>
              {i18n.t(item.layoutDetails?.address?.district || item.address)}
            </Text>
          )}
 ...
 <Text style={styles.textStyleNew}>{item.propertyDetails?.landDetails?.sell?.plotSize || item.size} {item.propertyDetails?.landDetails.sell?.sizeUnit}</Text> */}
              <Text style={styles.textStyleNew}>
                {item.propertyDetails?.landDetails?.lease?.plotSize ||
                  ...
                  item.size}{" "}
                {item.propertyDetails?.landDetails.lease?.sizeUnit || "Sq.ft"}
              </Text>
            </>
          )}
          ...
          {item.propertyType === "Agricultural land" && (
            <>
              <Text style={styles.textStyleNew}>
                {item.landDetails?.size || item.size}{" "}
                {item.landDetails?.sizeUnit || "Acres"}
              </Text>
            </>
          )}
          {(item.propertyType === "Residential" ||
            item.propertyType === "residential") && (
            <Text style={styles.textStyleNew}>
              {item.propertyDetails?.flatSize || item.size}{" "}
              {item.propertyDetails?.sizeUnit || "Sq.ft"}
            </Text>
          )}
          {item.propertyType === "Layout" && (
            <Text style={styles.textStyleNew}>
              {item.layoutDetails?.plotSize || item.size}{" "}
              {item.layoutDetails?.sizeUnit || "Sq.ft"}
            </Text>
          )}
        </View>
      </View>
        ...
        <View style={styles.centeredView}>
          <View style={styles.modalView}>
            <View style={styles.buttonDirection}>
              <Button title="Reset" style={{ textTransform: "none" }}></Button>
