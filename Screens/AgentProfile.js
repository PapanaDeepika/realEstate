...
<View colors={['#6A11CB', '#2575FC']} style={styles.header}>
          ...
          {/* <Text style={styles.name}>{`${profile.firstName} ${profile.lastName}  @ ${profile.accountId || profile._id}`} </Text> */}
          {language==="te"?(<Text style={styles.name}>{`${profile.firstNameTe || "జస్వంత్"} ${profile.lastNameTe || "కుమార్"}`}  @{` ${profile.accountId || profile._id}`}</Text>):(<Text style={styles.name}>{`${profile.firstName} ${profile.lastName}  @ ${profile.accountId || profile._id}`}</Text>)}
...
<View style={styles.userInfoSection}>
    ...
    <View style={styles.row}>
        ...
        <Text style={{color:"#333333", marginLeft:20}}>{ formatPhoneNumber(profile.phoneNumber)}</Text>
    </View>
    <View style={styles.row}>
