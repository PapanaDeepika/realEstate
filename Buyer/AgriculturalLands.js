...
function AgriculturalLands({ navigation }) {
...
const lang = isTelugu === 'English' ? 'en' : 'te'
  ...
  const fetchProperties = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch("http://172.17.15.189:3000/fields/getallfields", {
        method: "GET",
        headers: {
 ...
 <TouchableOpacity style={styles.cardNew} onPress={() => propertyDetails(item)} key={item._id}>
    ...
    {(item.propertyType === "Agricultural land" || item.propertyType === "Agricultural" ) &&
     (<ImageBackground style={styles.imageNew} source ={{uri:  item.landDetails?.images?.[0]
          ||
    ...
    <Text style={styles.textStyleNew}>{
     ...
     item.landDetails?.size } { 
      ...
      item.landDetails?.sizeUnit }</Text>
    </View>
    </View>
