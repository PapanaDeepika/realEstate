...
function InterestedProperties({ navigation }) {
  ...
  const fetchProperties = async () => {
    ...
    try {
      ...
      console.log(
        `https://real-estate-back-end-y58p-git-main-pindu123s-projects.vercel.app//deal/getIntrestedProperties?page=${page}&limit=8`
      );
      // const response = await fetch("https://real-estate-back-end-y58p-git-main-pindu123s-projects.vercel.app/deal/getIntrestedProperties", {
    ...
    >
      {(item.propertyType === "Agricultural land" ||
        ...
        >
          <Text style={styles.imageText}>
            {item.property?.landDetails?.title} @ {item.property?.propertyId}
          </Text>
          <Text style={styles.priceBottomStyle}>
            {formatPrice(item.property?.landDetails?.price)} /{" "}
            {item.property?.landDetails?.priceUnit}
          </Text>
        </ImageBackground>
      )}
      ...
      {item.propertyType === "Residential" && (
        ...
        >
          ...
          <Text style={styles.priceBottomStyle}>
            {formatPrice(item.property?.propertyDetails?.flatCost)}
          </Text>
        </ImageBackground>
      )}
          ...
          <Text style={styles.priceBottomStyle}>
            {formatPrice(
              item.property?.propertyDetails?.landDetails?.sell?.price ||
                ...
                item.property?.propertyDetails?.landDetails?.lease?.leasePrice
            )}{" "}
            /{" "}
      ...
      {item.propertyType === "Layout" && (
        ...
        >
          ...
          <Text style={styles.priceBottomStyle}>
            {formatPrice(item.property?.layoutDetails?.plotPrice)} /{" "}
            {item.property?.layoutDetails?.priceUnit}
          </Text>
        </ImageBackground>
      )}
...
const styles = StyleSheet.create({
  ...
  propertyList: {
    paddingHorizontal: 15,    fontFamily: "Montserrat_500Medium",
