...
import { View, Text, Image, StyleSheet, ScrollView, TouchableOpacity, ImageBackground, TextInput, ToastAndroid, ActivityIndicator } from 'react-native';
  ...
  const handleSubmit = async () => {
    ...
    if (validationErrors()) {
      try {
        ...
        const response = await axios.put(`https://real-estate-back-end-y58p-git-main-pindu123s-projects.vercel.app/auction/bid/${auctionProperty?._id}`, data, {
          headers: {
  ...
  const getAuctionDetails = async () => {
    try {
      ...
      const response = await fetch(
        `https://real-estate-back-end-y58p-git-main-pindu123s-projects.vercel.app/auction/getAuctionDetailsProperty/${property.propertyId}`, 
        {
          method: "GET",
          headers: {
            Authorization: `Bearer ${token}`,
        ...
        <>
          <View style={{ flexDirection: "column", backgroundColor: "black" }}>
            ...
            <TouchableOpacity style={styles.cardNew} onPress={() => propertyDetails(property)} key={property._id}>
                   ...
                   <Text style={styles.textStyleNew}>{i18n.t(auctionProperty.property?.layoutDetails?.address?.district || 
                   ...
                   auctionProperty.property?.landDetails?.size } {translateKey( auctionProperty.property?.layoutDetails?.sizeUnit || 
          ...
          <View style={styles.auctionDetailsContainer}>
            <Text style={styles.title}>{i18n.t("Auction Details")}</Text>
            <Text style={styles.text}>{i18n.t("Start Date")}: {formatDate(auctionProperty.startDate)}</Text>
            <Text style={styles.text}>{i18n.t("End Date")}: {formatDate(auctionProperty.endDate)}</Text>
            <Text style={styles.text}>{i18n.t("Initial Bid")}: ₹{auctionProperty.amount}</Text>
            {auctionProperty?.auctionType === "public" ? (
    <>
      <Text style={styles.text}>{i18n.t("Current Bid")}: ₹{auctionProperty.maximumBid?.bidAmount || auctionProperty.amount}</Text>
      ...
      <Text style={styles.warningText}>{i18n.t("Current Bid is displayed because this is a public auction")}</Text>
</View>
    </>
  ) : (
    <View style={styles.warningContainer}>
      <Icon name="alert" size={20} color="yellow" />
      <Text style={styles.warningText}>{i18n.t("Current Bid is not displayed because this is a private auction")}</Text>
    </View>
  )}
            <Text style={styles.footerText}>{translateKey("Now you can start participating in the auction. Start Bidding")}.</Text>

            <Text style={styles.rulesTitle}>{i18n.t("Rules and Regulations")}:</Text>
            <Text style={styles.rulesText}>- {i18n.t("The user entered amount should be paid")}.</Text>
            <Text style={styles.rulesText}>- {i18n.t("10% of the amount will be charged as a platform fee")}.</Text>
            <Text style={styles.rulesText}>-{i18n.t("If the user does not win the auction the amount will be refunded except for the platform fee")}.</Text>

            <Text style={styles.label}>{i18n.t("Enter Bid Amount")}</Text>
            <TextInput
              style={styles.input}
              value={bidAmount}
              onChangeText={handleChange}
              placeholder={i18n.t("Enter bid amount")}
            ...
            <View style={styles.checkboxContainer}>
              ...
              />
              <Text style={styles.checkboxLabel}>{i18n.t("Yes, I agree")}</Text>
            </View>
