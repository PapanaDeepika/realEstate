...
loadLanguage()
  ...
  const handleSearch = async () => {
    ...
    const results = properties.filter((prop) => {
      return (
        ...
        prop.property?.landDetails?.title
          ?.toLowerCase()
          .includes(lowercasedQuery) ||
        prop.property?.layoutDetails?.layoutTitle
          ?.toLowerCase()
          .includes(lowercasedQuery) ||
        prop.property?.propertyDetails?.apartmentName
          ?.toLowerCase()
          .includes(lowercasedQuery) ||
        prop.property?.propertyTitle?.toLowerCase().includes(lowercasedQuery) ||
        prop.property?.address?.district
          ?.toLowerCase()
          .includes(lowercasedQuery) ||
        prop.property?.address?.state
          ?.toLowerCase()
          .includes(lowercasedQuery) ||
        prop.property?.layoutDetails?.address?.district
          ?.toLowerCase()
          .includes(lowercasedQuery) ||
        prop.property?.layoutDetails?.address?.state
          ?.toLowerCase()
          .includes(lowercasedQuery) ||
        prop.property?.propertyDetails?.landDetails?.address?.district
          ?.toLowerCase()
          .includes(lowercasedQuery) ||
        prop.property?.propertyDetails?.landDetails?.address?.state
          ?.toLowerCase()
          .includes(lowercasedQuery)
      );
    });
  ...
  const getDealAssociatedProperties = useCallback(async () => {
    ...
    try {
      ...
      let url =
        "https://real-estate-back-end-y58p-git-main-pindu123s-projects.vercel.app/deal/getAgentDealings2";

      if (switched) {
        url += `/buyer`;
      } else {
        url =
          "https://real-estate-back-end-y58p-git-main-pindu123s-projects.vercel.app/deal/getAgentDealings";
      }
        ...
        <Card style={styles.card}>
          <Card.Content>
            {property.property.propertyType === "Agricultural land" && (
              <>
                <Text style={styles.customerName}>
                  {property.property?.landDetails?.title || "N/A"}
                </Text>
                <View style={styles.detailsContainer}>
                  ...
                  <View style={styles.detailItem}>
                    ...
                    />
                    <Text style={styles.detailText}>
                      {property.property?.address?.district || "N/A"},{" "}
                      {property.property?.address?.state || "N/A"}
                    </Text>
                  </View>
                </View>
              </>
            ...
            {property.property.propertyType === "Commercial" && (
              <>
                <Text style={styles.customerName}>
                  {property.property?.propertyTitle || "N/A"}
                </Text>
                <View style={styles.detailsContainer}>
                  <View style={styles.detailItem}>
                    <Feather name="at-sign" size={20} color="#057ef0" />
                  ...
                  <View style={styles.detailItem}>
                    ...
                    />
                    <Text style={styles.detailText}>
                      {property.property?.propertyDetails?.landDetails?.address
                        ?.district || "N/A"}
                      ,{" "}
                      {property.property?.propertyDetails?.landDetails?.address
                        ?.state || "N/A"}
            ...
            {property.property.propertyType === "Layout" && (
              <>
                <Text style={styles.customerName}>
                  {property.property?.layoutDetails?.layoutTitle || "N/A"}
                </Text>
                <View style={styles.detailsContainer}>
                  <View style={styles.detailItem}>
                    <Feather name="at-sign" size={20} color="#057ef0" />
                  ...
                  <View style={styles.detailItem}>
                    ...
                    <Text style={styles.detailText}>
                      {property.property?.layoutDetails?.address?.district ||
                        "N/A"}
                      ,{" "}
                      {property.property?.layoutDetails?.address?.state ||
                        "N/A"}
                    </Text>
                  </View>
                </View>
            ...
            {property.property.propertyType === "Residential" && (
              <>
                {language === "te" ? (
                  <Text style={styles.customerName}>
                    {property.property?.propertyDetails?.apartmentNameTe ||
                      property.property?.propertyDetails?.apartmentName}
                  </Text>
                ) : (
                  <Text style={styles.customerName}>
                    {property.property?.propertyDetails?.apartmentName || "N/A"}
                  </Text>
                )}
                <View style={styles.detailsContainer}>
                  <View style={styles.detailItem}>
                  ...
                  <View style={styles.detailItem}>
                    ...
                    />
                    <Text style={styles.detailText}>
                      {i18n.t(property.property?.propertyType) || "N/A"}
                    </Text>
                  </View>
                  <View style={styles.detailItem}>
                    <MaterialCommunityIcons
                    ...
                    />
                    <Text style={styles.detailText}>
                      {i18n.t(property.property?.address?.district) || "N/A"},{" "}
                      {property.property?.address?.state || "N/A"}
                    </Text>
                  </View>
                </View>
          ...
          >
            ...
            >
              <Text style={{ color: "white", fontSize: 16 }}>{i18n.t("View Agent")}</Text>
            </TouchableOpacity>
          </View>
        </Card>
      </>
            ...
            {selectedAgent ? (
              <View style={styles.agentContainer}>
                <View style={styles.centerContent}>
                  <Image
                    source={{ uri: selectedAgent?.profilePicture || "" }}
                    style={styles.agentProfileImage}
                  />
                  <Text style={styles.agentName}>
                    {selectedAgent?.firstName} {selectedAgent?.lastName}
                  </Text>
                  <Text style={styles.accountId}>
                    Account ID: {selectedAgent.accountId}
                  </Text>
                </View>
                ...
                <View style={styles.infoWrapper}>
                  ...
                  <View style={styles.infoRow}>
                    ...
                    />
                    <Text style={styles.infoText}>
                      {selectedAgent.mandal}, {selectedAgent.district},{" "}
                      {selectedAgent.city}, {selectedAgent.state},{" "}
                      {selectedAgent.country} - {selectedAgent.pinCode}
                    </Text>
                  </View>
                </View>
        ...
        <>
          ...
          >
            <TouchableOpacity
              ...
              onPress={() =>
                navigation.navigate("createDeal", { ba: switched })
              }
            >
              <Animated.View
                style={[
