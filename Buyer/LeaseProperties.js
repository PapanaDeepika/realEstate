...
function LeaseProperties({ navigation }) {
  ...
  const fetchProperties = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch("https://real-estate-back-end-y58p-git-main-pindu123s-projects.vercel.app/commercials/getProperties/lease", {
        method: "GET",
        headers: {
...
const checkinglanguage = useCallback(async () => {
 ...
 <TouchableOpacity style={styles.cardNew} onPress={() => propertyDetails(item)} key={item._id}>
    ...
    {(item.propertyType === "Agricultural land" || item.propertyType === "Agricultural" ) && (<ImageBackground style={styles.imageNew} source ={{uri: item.images?.[0] || item.landDetails?.images?.[0]
         || item.uploadPics?.[0] || item.propPhotos?.[0] ||
          "https://miro.medium.com/v2/resize:fit:800/1*PX_9ySeaKhNan-yPMW4WEg.jpeg"}}>
    ...
    {item.propertyType === "Commercial" && (<ImageBackground style={styles.imageNew} source ={{uri: item.propertyDetails?.uploadPics?.[0]  ||  "https://miro.medium.com/v2/resize:fit:800/1*PX_9ySeaKhNan-yPMW4WEg.jpeg"}}>
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.propertyListContainer}>
        ...
        {loading ? (
          ...
          <ActivityIndicator size="large" color="#007bff"  style={{marginVertical:200}}/></View>
        ) : (
          <FlatList
          scrollEnabled={false}
