...
function BuyProperties({ route }) {
  const navigation = useNavigation();
  console.log("ROUTEEE", route?.params?.type);
  ...
  const fetchProperties = async () => {
    ...
    try {
      ...
      // const response = await fetch(`https://real-estate-back-end-y58p-git-main-pindu123s-projects.vercel.app/commercials/getProperties/${type}`, {
      const response = await fetch(
        `https://real-estate-back-end-y58p-git-main-pindu123s-projects.vercel.app//commercials/getProperties/${type}?page=${page}&limit=8`,
        {
          method: "GET",
          headers: {
      ...
      if (data.data) {
        if (data.data.length < 4&& page!==1) {
          console.log("data", data.data.length);
          setHasMoreData(false);
        }
        setProperties((prevData) => [...prevData, ...data.data]);
        setFilteredProperties((prevData) => [...prevData, ...data.data]);
        setPage((prevPage) => prevPage + 1);
        setLoading(false);
        setRefreshing(false);
      } else {
        console.log("asdd", hasMoreData);
        setHasMoreData(false);
      }
    } catch (error) {
        ...
        <View style={styles.propertyListContainer}>
          {loading ? (
            ...
            <FlatList
              ...
              onEndReached={() => {
                if (hasMoreData && !loading1) {
                  console.log("Fetching more data...");
                  fetchProperties(); // Make sure this updates loading1
                }
              }}
